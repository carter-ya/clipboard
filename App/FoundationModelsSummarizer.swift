...
import PDFKit
  ...
  ///
  /// The type itself is only compiled when the SDK ships FoundationModels
  /// (Xcode 26+ / macOS 26 SDK). Older SDKs skip the symbol entirely so
  /// CI on macos-15 runners still builds; callers must also gate usage
  /// with `#if canImport(FoundationModels)`.
